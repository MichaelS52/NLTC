...
import me.toaster.nolimitstc.types.doombuggy.DoomBuggyStartupData;
import me.toaster.nolimitstc.types.doombuggy.DoomBuggyTrain;

public class DoomBuggyCommand  implements CommandExecutor{
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if(sender instanceof BlockCommandSender){
			...
			if(cmd.getLabel().equalsIgnoreCase("nltcdoombuggy")){
				...
				if(args.length==7){
					...
					try {
						DoomBuggyStartupData rcdata = new DoomBuggyStartupData(name, id, data, off, blk, blk, pass, waitTime);
						DoomBuggyTrain train = rcdata.getTrainFromData();
						if(train!=null){
