...
public class RollercoasterStartupData {
	...
	int stationWait = 0;
	public RollercoasterStartupData(String name, int id, int data, double offset, Block sender, Block track,
	...
	public RollercoasterTrain getTrainFromData(){
		try{
			File f = new File(Main.getPlugin(Main.class).getDataFolder(),name+".txt");
			if(f.exists()){
				return new RollercoasterTrain(f, offset, id, data, track, sender, pass, stationWait);
			}else{
